...
class Trade(db.Model):
    ...
    trade_date = db.Column(db.DateTime, default=datetime.today)
    strike = db.Column(db.Float(64),default=None)
    amount = db.Column(db.Float(64),default=None)
    price = db.Column(db.Float(64),default=None)
    user_id = db.Column(db.Integer, db.ForeignKey('users.id'))
    trade_date_ping = db.Column(db.DateTime, default=datetime.utcnow)
    
    def __repr__(self):
        return '<Trade %r>' % str(self.trade_date_ping)
    ...
    def payoff(self,central=None,i=0.001,n=1000):
        ...
        order = np.ones(end-start,float)*(-1.) if self.order.lower()=="sell" else np.ones(end-start,float)
...
class User(UserMixin, db.Model):
    ...
    trades = db.relationship('Trade', backref='trade_user', lazy='dynamic')
